...
Node* BST::deleteNode(Node *currNode, int value)
{
  ...
  //if key to be deleted is smaller than the roots key, it lies in the left subtree
  ...
  else
  {
    ...
    {
        Node* temp = currNode->right; //store value in temp
        delete currNode;
        return temp;
    ...
    else
    {
      //Replace with Minimum from right subtree
        Node* temp = getMinValueNode(currNode->right); //find minimum from right subtree
        currNode->key = temp->key; //copy the value
        currNode->right = deleteNode(currNode->right, temp->key); //delete the minimum from right
    }
