...
void Graph::DFTraversal(vertex *n)
{
    ...
    for(int x = 0; x < n->adj.size(); x++ )
    {
        ...
        if(!n->adj[x].v->visited){
            n->adj[x].v->visited = true; // ensures each vertex is visited only once
...
bool Graph::isBridge(int key1, int key2)
{
  ...
  removeEdge(key1, key2); //call function
  ...
  for(int i = 0; i < vertices.size(); i++){
    if(!vertices[i]->visited){ //if vertex has not been visited
      DFTraversal(vertices[i]);//call function
      components_after_removal++; //keep track of removed vertices
    }
  }
  ...
  if(components_after_removal > initial_components){ //check
    return true;
  }
